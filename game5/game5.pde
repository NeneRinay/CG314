...
//INSTRUCTIONS: Press any key to start. Then use left and right arrow keys to catch ingredients as they fall. Simple stuff.
...
void draw() {
  ...
  if (titleScreen == true) {
    ...
    //soundPlayer.play();
    soundPlayer = minim.loadFile("hellsKitchen.mp3", 2048);
  }


  if (titleScreen == false) {
    //soundPlayer.close();
    image (pinkKitchen, 0, 0, 550, 730); //this is the background
    //hitBun();
    reachBottom();
    player.display();
    player.move();
